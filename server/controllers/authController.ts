...
const authController = {
    ...
    refreshToken: async(req: Request, res: Response) => {
        try {
            ...
            if (!decoded.id) return res.status(400).json({ msg: "Please login first!" });

            let user = await Users.findById(decoded.id).select("-password +rf_token");
            if (!user) return res.status(400).json({ msg: "This account does not exist." });
            ...
            user = await Users.findOneAndUpdate({ _id: user._id }, {
